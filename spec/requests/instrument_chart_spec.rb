...
RSpec.describe "Instrument chart", type: :request do
  ...
  it "shows relative strength, its score and matching chart tooltip" do
    ...
    chart_levels = page.css("svg [data-chart-level-id]")
    expect(chart_levels.map { |node| node["data-level-strength"] }).to eq(%w[1 2 3])
    expect(chart_levels.map { |node| node["display"] }).to eq(%w[none none inline])
    choices = page.css("[data-chart-strength-filter-target='choice']")
    expect(choices.map { |node| node["value"] }).to eq(%w[3 2 1 unrated])
    expect(choices.select { |node| node["checked"] }.map { |node| node["value"] }).to eq(["3"])
    expect(page.at_css("[data-chart-strength-filter-target='summary']").text).to eq("Сила: высокая")
